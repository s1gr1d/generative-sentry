...
const ArtisticCubes: React.FC<ArtisticCubesProps> = ({
  ...
  const cubesData = useMemo<ArtisticCubeData[]>(() => {
    ...
    const materialTypes: ArtisticCubeData['materialType'][] = [
      'wobble', 'distort', 'reflection', 'transmission', 'holographic'
    ]
  ...
  useFrame((state) => {
    ...
    groupRef.current.children.forEach((cubeGroup, index) => {
      ...
      const floatX = Math.sin(time + cubeData.phaseOffset.x) * floatRadius * 25 + 
                    Math.cos(time * 0.7 + cubeData.phaseOffset.x) * floatRadius * 10
      const floatY = Math.cos(time * 0.8 + cubeData.phaseOffset.y) * floatRadius * 30 + 
                    Math.sin(time * 1.3 + cubeData.phaseOffset.y) * floatRadius * 8
      const floatZ = Math.sin(time * 0.6 + cubeData.phaseOffset.z) * floatRadius * 20 + 
      ...
      const rotationMultiplier = cubeData.materialType === 'wobble' ? 1.5 : 
                                cubeData.materialType === 'distort' ? 2.0 : 1.0
