...
export function Navigation() {
	...
	useEffect(() => {
		...
		const handleScroll = (scrollContainer: HTMLElement) => {
			...
			if (scrollTop <= 50) {
				// Small threshold for top boundary
			...
			if (import.meta.env.DEV) {
				console.log("Scroll Debug:", {
					...
					sections: Array.from(sectionElements).map((el) => ({
						...
						isInView:
							scrollPosition >= (el as HTMLElement).offsetTop &&
			...
			if (detectedSection && detectedSection !== activeSectionRef.current) {
				console.log(
					`🔄 Updating active section from "${activeSectionRef.current}" to "${detectedSection}"`,
				);
				...
				console.log(
					`⚪ No update needed - detected: "${detectedSection}", active: "${activeSectionRef.current}"`,
				);
			}
		};
