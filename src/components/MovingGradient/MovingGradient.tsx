...
const MovingGradient: React.FC<MovingGradientProps> = ({
	...
	const currentColors = useMemo(() => {
		...
		const fallbackColors: (keyof typeof COLOR_PALETTE)[] = [
			"BLURPLE",
			"DK_PURPLE",
			"LT_PINK",
			"DK_ORANGE",
		...
		return randomColors.slice(0, 5) as [
			keyof typeof COLOR_PALETTE,
			keyof typeof COLOR_PALETTE,
			keyof typeof COLOR_PALETTE,
			keyof typeof COLOR_PALETTE,
	...
	const shaderMaterial = useMemo(() => {
		const material = new THREE.ShaderMaterial({
			...
			uniforms: {
				...
				u_color1: {
					value: new THREE.Color(...(NORMALIZED_RGB[currentColors[0]] || [0.5, 0.3, 1])),
				},
				u_color2: {
					value: new THREE.Color(...(NORMALIZED_RGB[currentColors[1]] || [0.3, 0.05, 0.35])),
				},
				u_color3: {
					value: new THREE.Color(...(NORMALIZED_RGB[currentColors[2]] || [1, 0.3, 0.7])),
				},
				u_color4: {
					value: new THREE.Color(...(NORMALIZED_RGB[currentColors[3]] || [0.9, 0.5, 0.1])),
				},
				u_color5: {
					value: new THREE.Color(...(NORMALIZED_RGB[currentColors[4]] || [0.75, 0.9, 0.3])),
				},
				u_speed: { value: speed },
	...
	useEffect(() => {
		if (shaderMaterial) {
			shaderMaterial.uniforms.u_color1.value.set(
				...(NORMALIZED_RGB[currentColors[0]] || [0.5, 0.3, 1]),
			);
			shaderMaterial.uniforms.u_color2.value.set(
				...(NORMALIZED_RGB[currentColors[1]] || [0.3, 0.05, 0.35]),
			);
			shaderMaterial.uniforms.u_color3.value.set(
				...(NORMALIZED_RGB[currentColors[2]] || [1, 0.3, 0.7]),
			);
			shaderMaterial.uniforms.u_color4.value.set(
				...(NORMALIZED_RGB[currentColors[3]] || [0.9, 0.5, 0.1]),
			);
			shaderMaterial.uniforms.u_color5.value.set(
				...(NORMALIZED_RGB[currentColors[4]] || [0.75, 0.9, 0.3]),
			);
		}
	...
	useFrame((state) => {
		if (shaderMaterial) {
			...
			if (animateColors) {
				...
				if (colorChangeTimeRef.current > 5.0) {
