	...
	return (
		<div className={`${styles.container} ${className || ""}`}>
			<div className={styles.header}>
				...
				<p>
					Generated {SAMPLE_SENTRY_ENVELOPES.length} error envelopes across{" "}
					{Object.keys(GROUPED_ERRORS_BY_TYPE).length} error types
				</p>
			</div>
				...
				<div className={styles.modal} onClick={() => setSelectedEnvelope(null)}>
					<div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
						<div className={styles.modalHeader}>
							<h3>Error Details</h3>
							<button className={styles.closeButton} onClick={() => setSelectedEnvelope(null)}>
								×
							</button>
						</div>
						<div className={styles.modalBody}>
							...
							<div className={styles.detailRow}>
								<strong>Location:</strong> {selectedEnvelope.user?.geo?.city},{" "}
								{selectedEnvelope.user?.geo?.country_code}
							</div>
							{selectedEnvelope.exception?.values[0] && (
								<div className={styles.exceptionDetails}>
									...
									<div className={styles.exception}>
										<div>
											<strong>Type:</strong> {selectedEnvelope.exception.values[0].type}
										</div>
										<div>
											<strong>Value:</strong> {selectedEnvelope.exception.values[0].value}
										</div>
