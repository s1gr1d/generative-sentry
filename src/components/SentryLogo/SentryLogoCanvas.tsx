...
import { useGLTF, MeshDistortMaterial, MeshWobbleMaterial, GradientTexture, GradientType, Text3D, OrbitControls } from '@react-three/drei'
import { EffectComposer, Noise, DepthOfField, Bloom, Vignette, ChromaticAberration } from '@react-three/postprocessing'
import { BlendFunction } from 'postprocessing'
import * as THREE from 'three'
import { PRIMARY_COLORS, SECONDARY_COLORS, NORMALIZED_RGB, getRandomColors } from '@/utils/colorPalette'
...
function SentryLogo({ scale = 1, autoRotate = true, rotationSpeed = 0.5 }: SentryLogoCanvasProps) {
  ...
  const originalMeshes = useRef<Array<{ mesh: THREE.Mesh; originalMaterial: THREE.Material | THREE.Material[] }>>([])
  
  // Collect all meshes from the GLTF model and store their original materials
  useEffect(() => {
    console.log('Cloned scene:', clonedScene)
    if (clonedScene) {
      const meshes: Array<{ mesh: THREE.Mesh; originalMaterial: THREE.Material | THREE.Material[] }> = []
  ...
  const SentryLogoMeshes = () => {
    console.log('SentryLogoMeshes rendering - clonedScene:', !!clonedScene, 'meshes:', originalMeshes.current.length, 'meshesReady:', meshesReady)
    
    if (!clonedScene || !meshesReady || originalMeshes.current.length === 0) {
      console.log('No cloned scene or meshes available - scene:', !!clonedScene, 'meshes:', originalMeshes.current.length, 'ready:', meshesReady)
      return null
    }
...
function WobblyBall({ data }: { data: WobblyBallData }) {
  ...
  useFrame((state) => {
    ...
    ballRef.current.position.x = data.position.x + Math.sin(time * 0.8 + data.phaseOffset.x) * 2 + time * data.drift.x
    ballRef.current.position.y = data.position.y + Math.cos(time * 0.6 + data.phaseOffset.y) * 1.5 + time * data.drift.y
    ballRef.current.position.z = data.position.z + Math.sin(time * 0.4 + data.phaseOffset.z) * 1 + time * data.drift.z
  ...
  return (
    ...
    >
      ...
      <directionalLight 
        position={[-5, 5, 3]} 
        intensity={0.2}
        color={PRIMARY_COLORS.LT_BLURPLE}
      />
      <pointLight 
        position={[-10, -10, -5]} 
        intensity={0.8}
      ...
      <pointLight 
        position={[5, 5, -3]} 
        intensity={2}
        color={SECONDARY_COLORS.DK_PINK}
      />
      <pointLight 
        position={[0, 10, 5]} 
        intensity={0.5}
        color={SECONDARY_COLORS.LT_YELLOW}
      />
