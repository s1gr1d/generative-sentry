...
const SpanAnalyticsStats = ({ className }: SpanAnalyticsStatsProps) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'operations' | 'projects' | 'insights'>('overview');
  ...
  const getSeverityColor = (severity: 'low' | 'medium' | 'high') => {
    switch (severity) {
      case 'high': return SECONDARY_COLORS.DK_PINK;
      case 'medium': return SECONDARY_COLORS.DK_ORANGE;
      case 'low': return TERTIARY_COLORS.DK_GREEN;
      default: return COLOR_PALETTE.RICH_BLACK;
    }
  };
    ...
    <div className={styles.overviewGrid}>
      <div className={styles.statCard}>
        ...
        <div className={styles.statRow}>
          ...
          <span className={styles.statValue}>{formatDuration(analytics.durationStats.percentiles.p95)}</span>
        </div>
        <div className={styles.statRow}>
          <span>Standard Deviation:</span>
          <span className={styles.statValue}>{formatDuration(analytics.durationStats.standardDeviation)}</span>
        </div>
      </div>
      ...
      <div className={styles.statCard}>
        ...
        <div className={styles.statRow}>
          ...
          <span className={styles.statValue}>{Object.keys(analytics.operationStats.frequency).length}</span>
        </div>
        <div className={styles.statRow}>
          <span>Projects:</span>
          <span className={styles.statValue}>{Object.keys(analytics.projectStats.distribution).length}</span>
        </div>
        <div className={styles.statRow}>
          <span>Time Range:</span>
          <span className={styles.statValue}>{formatDuration(analytics.temporalStats.timeRange.duration)}</span>
        </div>
      </div>
    </div>
  );
        ...
        <div className={styles.operationsList}>
          {analytics.operationStats.performanceRank.slice(0, 12).map((op, index) => (
            <div 
              ...
              onClick={() => setSelectedOperation(selectedOperation === op.operation ? null : op.operation)}
        ...
        <div className={styles.operationDetails}>
          ...
          <div className={styles.operationDetailStats}>
            <div className={styles.detailItem}>
              ...
              <span>{formatDuration(analytics.operationStats.avgDuration[selectedOperation as keyof typeof analytics.operationStats.avgDuration])}</span>
            </div>
            <div className={styles.detailItem}>
              <span>Total Duration:</span>
              <span>{formatDuration(analytics.operationStats.totalDuration[selectedOperation as keyof typeof analytics.operationStats.totalDuration])}</span>
            </div>
            <div className={styles.detailItem}>
              <span>Frequency:</span>
              <span>{analytics.operationStats.frequency[selectedOperation as keyof typeof analytics.operationStats.frequency]} calls</span>
            </div>
          </div>
          ...
          <div className={styles.patternsList}>
            {analytics.insights.patterns.map((pattern, index) => (
              <div key={index} className={styles.patternItem}>
                ...
                <div className={styles.patternFreq}>
              ...
              return (
                <div key={hour} className={styles.hourBar}>
                  <div 
                    ...
                    style={{ 
                      height: `${height}%`,
                      backgroundColor: isPeak ? SECONDARY_COLORS.DK_PINK : PRIMARY_COLORS.LT_BLURPLE
                    }}
                  />
