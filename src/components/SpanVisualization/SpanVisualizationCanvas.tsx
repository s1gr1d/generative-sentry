...
import { OrbitControls, Environment, ContactShadows, Text3D, Float, Html } from "@react-three/drei";
...
const SpanParticle = ({ span, index }: SpanParticleProps) => {
	...
	useFrame((state) => {
		...
		meshRef.current.position.set(x + noiseX, y + noiseY, z + noiseZ);
...
const PerformanceLayerIndicator = ({
	layer,
	label,
	y,
}: {
	...
	const color =
		PERFORMANCE_LAYERS[layer as keyof typeof PERFORMANCE_LAYERS]?.color || COLOR_PALETTE.RICH_BLACK;
			...
			{operations.map((operation, index) => {
				...
				return (
					<group key={operation} position={[0, y, 0]}>
						...
						<Text3D font="/fonts/rubik-bold.json" size={0.3} height={0.05} position={[0, -0.1, 0]}>
							{operation}
	...
	return (
		<>
			...
			{visualizationSpans.map((span, index) => (
				<SpanParticle key={`${span.span_id}-${index}`} span={span} index={index} />
			))}

			{/* Ground plane with subtle contact shadows */}
			<ContactShadows position={[0, -2, 0]} opacity={0.2} scale={50} blur={2} far={10} />
