import { generateSentryEnvelopes, generateSentryEnvelopesForType, getErrorTypeDistribution } from '@/utils/sentryDataGenerator';
...
export const AUTHENTICATION_ERRORS: SentryEnvelope[] = generateSentryEnvelopesForType('authentication', 8);
...
export const RECENT_ERRORS: SentryEnvelope[] = SAMPLE_SENTRY_ENVELOPES
  .filter(envelope => {
...
export const HIGH_FREQUENCY_ERRORS: SentryEnvelope[] = SAMPLE_SENTRY_ENVELOPES
  .slice(0, 30) // Most recent errors tend to be high frequency
  .map(envelope => ({
    ...envelope,
...
export const getErrorsByEnvironment = (environment: 'production' | 'staging' | 'development'): SentryEnvelope[] => {
...
export const generateTimeSeriesData = (days: number = 7): Array<{date: string, errors: SentryEnvelope[]}> => {
  const result = [];
  const now = new Date();
...
export const ERROR_STATISTICS = {
  ...
  mostCommonErrorType: Object.entries(ERROR_DISTRIBUTION)
    .reduce((a, b) => a[1] > b[1] ? a : b)[0] as ErrorType,
