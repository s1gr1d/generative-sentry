...
export const HOMEPAGE_SPANS: SpanData[] = [
  ...
  ...HTTP_CLIENT_SPANS.filter(span => span.description.includes('homepage') || span.description.includes('GET /')),
...
export const DASHBOARD_SPANS: SpanData[] = [
  ...
  ...DB_QUERY_SPANS.filter(span => span.description.includes('SELECT') && span.description.includes('projects'))
];
...
export const MEDIUM_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => span.duration >= 20 && span.duration < 100);
export const SLOW_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => span.duration >= 100 && span.duration < 500);
export const VERY_SLOW_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => span.duration >= 500);

// Project-specific datasets
export const FRONTEND_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => 
...
export const BACKEND_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => 
...
export const PRODUCTION_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => span.tags?.environment === 'production');
export const STAGING_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => span.tags?.environment === 'staging');
export const DEVELOPMENT_SPANS: SpanData[] = SAMPLE_SPANS.filter(span => span.tags?.environment === 'development');
...
export const RECENT_SPANS: SpanData[] = SAMPLE_SPANS
...
export const ALL_DB_SPANS: SpanData[] = [...DB_QUERY_SPANS, ...SPANS_BY_OPERATION['db.transaction'], ...SPANS_BY_OPERATION['db.connection']];
export const ALL_CACHE_SPANS: SpanData[] = [...SPANS_BY_OPERATION['cache.get'], ...SPANS_BY_OPERATION['cache.set'], ...SPANS_BY_OPERATION['cache.miss']];
...
export const SPAN_STATISTICS = {
  ...
  averageDuration: Math.round(SAMPLE_SPANS.reduce((sum, span) => sum + span.duration, 0) / SAMPLE_SPANS.length),
...
export const getSpansInTimeRange = (startTime: number, endTime: number): SpanData[] => {
  return SAMPLE_SPANS.filter(span => 
    span.start_timestamp >= startTime && span.end_timestamp <= endTime
  );
};
