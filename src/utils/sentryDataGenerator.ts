...
const ERROR_TEMPLATES: Record<ErrorType, ErrorTemplate> = {
	...
	database: {
		...
		commonFunctions: ["query", "connect", "transaction", "findById", "create", "update"],
		commonModules: [
	...
	syntax: {
		...
		commonFunctions: ["parse", "eval", "compile", "transpile", "validate", "transform"],
		commonModules: [
			"app/utils/parser.ts",
	...
	return {
		...
		contexts: {
			...
			device: {
				...
				cpu_description: getRandomElement([
			...
			runtime: {
				runtime:
					template.platform === "node" ? "node v20.10.0" : `${browser.name} ${browser.version}`,
...
export const generateSentryEnvelopesForType = (
	errorType: ErrorType,
	count: number = 10,
): SentryEnvelope[] => {
	const envelopes: SentryEnvelope[] = [];
...
export const getErrorTypeDistribution = (
	...
	envelopes.forEach((envelope) => {
		...
		else if (errorType?.includes("database") || errorType?.includes("query"))
			distribution.database++;
		else if (errorType?.includes("auth") || errorType?.includes("token"))
			distribution.authentication++;
		else if (errorType?.includes("validation") || errorType?.includes("schema"))
			distribution.validation++;
		else if (errorType?.includes("permission") || errorType?.includes("access"))
