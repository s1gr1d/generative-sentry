...
export const analyzeSpanData = (spans: SpanData[] = SAMPLE_SPANS, traces: TraceData[] = SAMPLE_TRACES): SpanAnalytics => {
  ...
  // Performance ranking
  const performanceRank = Object.entries(operationFreq).map(([op, freq]) => {
    const operation = op as SpanOperation;
  ...
  const patterns = [
    ...
    },
    {
      ...
      frequency: Math.min(
        operationFreq['db.connection'] || 0,
        operationFreq['db.query'] || 0
      )
    }
  ];
  ...
  const durationOutliers = spans.filter(span => 
    Math.abs(span.duration - mean) > 3 * standardDeviation
  ).length;
...
export const getAnalyticsUpdate = (spans: SpanData[]): Partial<SpanAnalytics> => {
  const recentSpans = spans.filter(span => 
    span.start_timestamp > Date.now() - (5 * 60 * 1000) // Last 5 minutes
  );
