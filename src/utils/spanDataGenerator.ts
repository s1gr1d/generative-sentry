...
export const generateSpan = (
	...
	const statuses: SpanData["status"][] = [
		"ok",
		"ok",
		"ok",
		"ok",
	...
	return {
		...
		data: {
			"http.method": operation.startsWith("http")
				? getRandomElement(["GET", "POST", "PUT", "DELETE", "PATCH"])
				: undefined,
			"http.status_code": operation.startsWith("http")
				? getRandomElement([200, 201, 204, 400, 401, 403, 404, 500])
				: undefined,
			"db.statement": operation.startsWith("db") ? description : undefined,
			component: operation.startsWith("ui")
				? getRandomElement(["Button", "Form", "Chart", "Modal", "Table"])
				: undefined,
...
export const generateSpansForOperation = (
